// /api/search  —  유튜브 검색 프록시 (API 키는 서버에만 보관)
// 성과(조회수·좋아요·최신성)를 종합해 상위 영상 순으로 정렬해 반환한다.

const YT = "https://www.googleapis.com/youtube/v3";

function parseDuration(iso) {
  // PT1H2M3S -> {seconds, text}
  const m = /PT(?:(\d+)H)?(?:(\d+)M)?(?:(\d+)S)?/.exec(iso || "");
  const h = +(m?.[1] || 0), mi = +(m?.[2] || 0), s = +(m?.[3] || 0);
  const seconds = h * 3600 + mi * 60 + s;
  const text = h ? `${h}:${String(mi).padStart(2, "0")}:${String(s).padStart(2, "0")}`
                 : `${mi}:${String(s).padStart(2, "0")}`;
  return { seconds, text };
}

// 성과 점수: 조회수(로그) 중심 + 좋아요 비율 + 최신성 약간
function performanceScore(v) {
  ...
  const likeRatio = views > 0 ? likes / views : 0;    // 0~0.1 정도
  const freshness = 1 / Math.log10(ageDays + 10);     // 최신일수록 소폭 가산
  return viewScore + likeRatio * 20 + freshness;
}

export default async function handler(req, res) {
  const key = process.env.YOUTUBE_API_KEY;
  if (!key) {
    return res.status(500).json({ error: "YOUTUBE_API_KEY가 설정되지 않았습니다. Vercel 환경변수를 확인하세요." });
  }

  const url = new URL(req.url, "http://localhost");
  const q = (url.searchParams.get("q") || "").trim();
  const order = url.searchParams.get("order") || "performance"; // performance | views | latest | relevance
  ...
  try {
    ...
    const sp = new URLSearchParams({
      key, part: "snippet", q: searchQuery, type: "video",
      maxResults: "25", order: ytOrder, regionCode: region,
      relevanceLanguage: lang, videoEmbeddable: "true", safeSearch: "moderate",
    });
    ...
    if (order === "latest") {
      ...
      items.sort((a, b) => b._score - a._score); // performance
    }
